...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('SOAP Request XML:', soapXml) // Debug logging
    console.log('Sending to URL:', serverUrl) // Debug logging
    ...
    console.log('SOAP Response Status:', soapResponse.status) // Debug logging
    ...
    console.log('SOAP Response:', responseText) // Debug logging

    // Parse the SOAP response to extract the login result
    // Handle both self-closing tags and empty elements
    const loginResultMatch = responseText.match(/<LoginResult[^>]*>([^<]*)<\/LoginResult>|<LoginResult[^>]*\/>/)
    
    if (!loginResultMatch) {
      // Check for SOAP faults
      const faultMatch = responseText.match(/<soap:Fault>.*?<faultstring>(.*?)<\/faultstring>.*?<\/soap:Fault>/)
      if (faultMatch) {
        return NextResponse.json(
    ...
    response.cookies.set('auth_token', loginResult, {
      httpOnly: true,
